...
CREATE VIEW view_emp_dep_jobs AS (
    SELECT 
        e.employee_id,
        first_name || ' ' || last_name AS full_name,
        d.department_name,
...
CREATE OR REPLACE VIEW view_emp_dep_jobs AS (
    SELECT 
        e.employee_id,
        first_name || ' ' || last_name AS full_name,
        d.department_name,
...
SELECT * FROM view_emp_dep_jobs;

SELECT
    job_title,
    AVG(salary) AS avg
FROM view_emp_dep_jobs
GROUP BY job_title
